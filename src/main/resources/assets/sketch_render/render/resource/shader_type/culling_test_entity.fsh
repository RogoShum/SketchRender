...
void main() {
    ...
    if (sketch_testEntityPos.w > 0) {
        ...
        if (nearEdge) {
            fragColor = vec4(1.0, 0.0, 0.0, 0.35); // red outline for entities
            return;
        }
        // --------- Wireframe drawing logic end ---------

        ClipResult clip = getClippedMinDepth(entityPos + vec3(0, sketch_testEntityAABB.y / 2, 0), 
        ...
        if(all(greaterThanEqual(gl_FragCoord.xy - vec2(0.5), vec2(aabbMinScreen))) &&
           all(lessThanEqual(gl_FragCoord.xy - vec2(0.5), vec2(aabbMaxScreen)))) {
            fragColor = vec4(1.0, 0.0, 0.0, 0.35); // red for entities

...
public class ScissorState implements RenderStateComponent {
    ...
    public ScissorState(boolean enabled, int x, int y, int width, int height) {
        this.enabled = enabled; this.x = x; this.y = y; this.width = width; this.height = height;
    }
    ...
    public void apply(RenderStateComponent prev) {
        ScissorState p = prev instanceof ScissorState ? (ScissorState) prev : null;
        if (p == null || enabled != p.enabled) {
            if (enabled) GL11.glEnable(GL11.GL_SCISSOR_TEST);
            else GL11.glDisable(GL11.GL_SCISSOR_TEST);
        }
        if (enabled && (p == null || x != p.x || y != p.y || width != p.width || height != p.height)) {

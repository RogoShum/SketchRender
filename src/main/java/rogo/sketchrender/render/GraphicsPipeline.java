...
import rogo.sketchrender.util.OrderedList;
...
public class GraphicsPipeline<C extends RenderContext> {
    ...
    public boolean registerStage(GraphicsStage stage) {
        boolean added = stages.add(stage, stage.getOrderRequirement(), (s, req) -> passMap.put(s, new GraphicsPassGroup<>()));
        if (added) {
    ...
    public void renderStagesBetween(Identifier fromId, Identifier toId) {
        ...
        for (int i = fromIdx + 1; i < toIdx; i++) {
            GraphicsPassGroup<C> pass = passMap.get(ordered.get(i));
            if (pass != null) pass.render(currentContext);
        }
    }
    ...
    public void renderStage(Identifier id) {
        GraphicsStage stage = idToStage.get(id);
        if (stage != null) {
            GraphicsPassGroup<C> pass = passMap.get(stage);
            if (pass != null) {
                pass.render(currentContext);
            }
        }
    }

...
void main() {
    ...
    if (sketch_testBlockEntityPos.w > 0) {
        ...
        vec3 blockEntityPos = sketch_testBlockEntityPos.xyz;
        vec3 boxMin = blockEntityPos - vec3(sketch_testBlockEntityAABB.x / 2, 0, sketch_testBlockEntityAABB.z / 2);
        vec3 boxMax = blockEntityPos + vec3(sketch_testBlockEntityAABB.x / 2, sketch_testBlockEntityAABB.y, sketch_testBlockEntityAABB.z / 2);

        if(!isVisible(blockEntityPos, boxMin, boxMax)) {
            fragColor = vec4(0.0, 0.0, 0.0, 0.0);
            return;
        }
        ...
        if (nearEdge) {
            fragColor = vec4(0.0, 0.0, 1.0, 0.35); // blue outline for block entities
            return;
        }
        // --------- Wireframe drawing logic end ---------

        ClipResult clip = getClippedMinDepth(blockEntityPos + vec3(0, sketch_testBlockEntityAABB.y / 2, 0), 
        ...
        if(all(greaterThanEqual(gl_FragCoord.xy - vec2(0.5), vec2(aabbMinScreen))) &&
           all(lessThanEqual(gl_FragCoord.xy - vec2(0.5), vec2(aabbMaxScreen)))) {
            fragColor = vec4(0.0, 0.0, 1.0, 0.35); // blue for block entities

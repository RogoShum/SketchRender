...
void main() {
    fragColor = vec4(0.0, 0.0, 0.0, 0.0);
    ...
    #ifdef BLOCK_ENTITY
        if (sketch_testBlockEntityPos.w > 0) {
    #endif
    ...
    #ifdef CHUNK
        vec3 targetPos = sketch_testPos.xyz * CHUNK_SIZE + vec3(8.0);
        vec3 boxMin = targetPos - vec3(9.0);
        vec3 boxMax = targetPos + vec3(9.0);
    #endif

    #ifdef BLOCK_ENTITY
        vec3 targetPos = sketch_testBlockEntityPos.xyz;
        vec3 boxMin = targetPos - vec3(sketch_testBlockEntityAABB.x / 2, 0, sketch_testBlockEntityAABB.z / 2);
        vec3 boxMax = targetPos + vec3(sketch_testBlockEntityAABB.x / 2, sketch_testBlockEntityAABB.y, sketch_testBlockEntityAABB.z / 2);
    #endif

    #ifdef ENTITY
        vec3 targetPos = sketch_testEntityPos.xyz;
        vec3 boxMin = targetPos - vec3(sketch_testEntityAABB.x / 2, 0, sketch_testEntityAABB.z / 2);
        vec3 boxMax = targetPos + vec3(sketch_testEntityAABB.x / 2, sketch_testEntityAABB.y, sketch_testEntityAABB.z / 2);
    #endif

        if (!isVisible(targetPos, boxMin, boxMax)) {
            fragColor = vec4(0.0, 0.0, 0.0, 0.0);
            return;
        }
        ...
        #ifdef CHUNK
            ClipResult clip = getClippedMinDepth(targetPos + sketch_cullingCameraDir, 10.0, 10.0);
        #endif

        #ifdef BLOCK_ENTITY
            ClipResult clip = getClippedMinDepth(targetPos + vec3(0, sketch_testBlockEntityAABB.y / 2, 0),
                                                sketch_testBlockEntityAABB.x / 2 + 0.15,
                                                sketch_testBlockEntityAABB.y / 2 + 0.15);
        #endif

        #ifdef ENTITY
            ClipResult clip = getClippedMinDepth(targetPos + vec3(0, sketch_testEntityAABB.y / 2, 0),
                                                sketch_testEntityAABB.x / 2 + 0.15,
                                                sketch_testEntityAABB.y / 2 + 0.15);
        #endif
        ...
        all(lessThanEqual(gl_FragCoord.xy - vec2(0.5), vec2(aabbMaxScreen)))) {
            fragColor = vec4(0.0, 1.0, 0.0, 0.35); // green for chunks

...
/**
 * A VBO component that combines an OpenGL vertex buffer object with its layout and
 * binding configuration.
 * Represents a single vertex buffer binding point in an OpenGL geometry binding (VAO).
 *
 * <p>
 ...
 * </ul>
 */
public class OpenGLVertexComponent {
    private final OpenGLVertexBufferObject vbo;
     ...
     */
    public OpenGLVertexComponent(OpenGLVertexBufferObject vbo, ComponentSpec spec) {
        this(vbo, spec, false, 0, 0);
    }

    /**
     * Create a mutable internal VBO component (owned by the geometry binding, needs
     * filling).
     */
    public OpenGLVertexComponent(OpenGLVertexBufferObject vbo, KeyId id, StructLayout format, int bindingPoint, boolean instanced) {
     ...
     */
    public static OpenGLVertexComponent immutable(OpenGLVertexBufferObject vbo, KeyId id, StructLayout format,
            int bindingPoint, boolean instanced) {
        return new OpenGLVertexComponent(vbo, ComponentSpec.immutable(id, bindingPoint, format, instanced), false, 0, 0);
    }

    /**
     * Create an external VBO component (not owned by the geometry binding).
     * Used for zero-copy BakedMesh integration where the VBO is referenced from
     * elsewhere.
     */
    public OpenGLVertexComponent(int externalVboHandle, KeyId id, StructLayout format, int bindingPoint,
     ...
     */
    public static OpenGLVertexComponent external(OpenGLVertexComponent other) {
        // Create a copy that is marked external
        return new OpenGLVertexComponent(null, other.getSpec(), true, other.getVertexOffset(), other.getVboHandle());
    }

    private OpenGLVertexComponent(OpenGLVertexBufferObject vbo, ComponentSpec spec, boolean external, int vertexOffset, int externalVboHandle) {
        this.vbo = vbo;
        this.spec = spec;
    ...
    public OpenGLVertexBufferObject getVBO() {
        return vbo;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        OpenGLVertexComponent that = (OpenGLVertexComponent) o;

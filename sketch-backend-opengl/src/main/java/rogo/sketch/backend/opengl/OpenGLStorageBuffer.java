...
public class OpenGLStorageBuffer implements DataResourceObject, Resizeable,
        BackendInstalledBuffer, BackendInstalledBindableResource, BackendStorageBuffer {
    ...
    public OpenGLStorageBuffer(long dataCount, long stride, int usage) {
        this(dataCount, stride, MemoryUtil.nmemCalloc(dataCount, stride), usage);
    }

    public OpenGLStorageBuffer(long dataCount, long stride, long memoryAddress, int usage) {
        ...
        this.descriptor = new ResolvedBufferResource(
                KeyId.of("ssbo_" + System.identityHashCode(this)),
                BufferRole.STORAGE,
                usage == GL15.GL_STATIC_DRAW ? BufferUpdatePolicy.IMMUTABLE : BufferUpdatePolicy.DYNAMIC,
                dataCount,
                stride,
                totalCapacity);
    ...
    public OpenGLStorageBuffer(DataResourceObject buffer) {
    ...
    public void upload(long sourceAddress, long byteCount) {
        ...
        getBufferStrategy().bufferSubData(id, 0, byteCount, sourceAddress);
    }
    ...
    public void resetUpload(BufferUpdatePolicy updatePolicy) {
        resetUpload(toGLUsage(updatePolicy));
    }

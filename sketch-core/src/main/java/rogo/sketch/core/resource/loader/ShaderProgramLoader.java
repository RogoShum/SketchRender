...
import rogo.sketch.core.shader.GraphicsShader;
...
public class ShaderProgramLoader implements ResourceLoader<ShaderProvider> {
    ...
    @Override
    public ShaderProvider load(KeyId keyId, ResourceData data, Gson gson, Function<KeyId, Optional<InputStream>> resourceProvider) {
        try {
     ...
     */
    private Function<KeyId, Optional<InputStream>> createShaderResourceProvider(Function<KeyId, Optional<InputStream>> baseResourceProvider) {
        return (shaderId) -> {

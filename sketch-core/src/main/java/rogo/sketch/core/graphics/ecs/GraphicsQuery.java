...
/**
 * Simple ECS query over required and excluded component sets.
 */
public final class GraphicsQuery {
    private final Set<GraphicsComponentType<?>> required;
    private final Set<GraphicsComponentType<?>> excluded;

    private GraphicsQuery(Set<GraphicsComponentType<?>> required, Set<GraphicsComponentType<?>> excluded) {
    ...
    public boolean matches(Set<GraphicsComponentType<?>> signature) {
        return signature.containsAll(required) && Collections.disjoint(signature, excluded);
    }
    ...
    public static final class Builder {
        ...
        public GraphicsQuery build() {
            return new GraphicsQuery(required, excluded);
        }
    }
}

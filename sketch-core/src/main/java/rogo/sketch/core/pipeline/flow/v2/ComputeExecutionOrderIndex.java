...
public final class ComputeExecutionOrderIndex {
    public List<ComputeInstanceStore.Entry> order(Collection<ComputeInstanceStore.Entry> sourceEntries) {
        List<ComputeInstanceStore.Entry> ordered = new ArrayList<>(sourceEntries);
        ordered.sort(Comparator
                .comparingInt((ComputeInstanceStore.Entry entry) -> DefaultBatchContainers.orderOf(entry.containerType()))
                .thenComparing(this::compareWithinContainer));
        return ordered;
    }

    private int compareWithinContainer(ComputeInstanceStore.Entry left, ComputeInstanceStore.Entry right) {
